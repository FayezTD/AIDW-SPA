...
const extractGraphData = (content, marker) => {
  ...
  const endMarker =
    marker === "%%TABLE_JSON%%" ? "%%END_TABLE%%" : "%%END_GRAPH%%";
...
const RichContent = ({ content, mapData }) => {
  ...
  const graphData = useMemo(
    () => extractGraphData(content, "%%GRAPH_JSON%%"),
    [content]
  );
  const tableData = useMemo(
    () => extractGraphData(content, "%%TABLE_JSON%%"),
    [content]
  );
...
const generateVisualizationData = (content) => {
  ...
  while (
    (match = wordRegex.exec(content)) !== null &&
    labels.length < uniqueNumbers.length
  ) {
    labels.push(match[2]);
  }
...
const ChatMessage = ({
  ...
  let { role, content, timestamp, citations, hyperlinks, intent, mapData } =
    message;
  ...
  const hasSpecialContent =
    content &&
    (content.includes("%%TABLE_JSON%%") || content.includes("%%GRAPH_JSON%%"));
  ...
  const formatTextForSpeech = (text) => {
    return (
      text
        ...
        .replace(
          /%%TABLE_JSON%%.*?%%END_TABLE%%/gs,
          "Here is a table with data."
        )
        .replace(
          /%%GRAPH_JSON%%.*?%%END_GRAPH%%/gs,
          "Here is a graph visualization."
        )
        ...
        <div className="flex items-center mb-2">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center ${
              isUser ? "bg-cyan-700 text-cyan-100" : "bg-cyan-100 text-cyan-900"
            }`}
          >
            {isUser ? "U" : "AI"}
          </div>
          ...
          {timestamp && (
            <div className="ml-auto text-xs opacity-75">{formattedTime}</div>
          )}
          {intent && (
            <div className="ml-2 text-xs bg-gray-200 text- px-2 py-1 rounded-full">
              {intent}
            </div>
          )}
        </div>
  ...
  <>
    ...
    {mapData && (
      (
        ...
        // Handle series-based format
        (mapData.xAxis && 
         mapData.xAxis.data && 
         mapData.xAxis.data.length > 0 && 
         mapData.series && 
         Array.isArray(mapData.series) && 
         mapData.series.length > 0)
      ) && (
